...
describe("root lifecycle state machine", () => {
  ...
  it("allows backend result states when React batches the checking state", () => {
    ...
    expect(transition(restoring, { lifecycle: "no_workspace" }).lifecycle).toBe(
      "no_workspace",
    );
    expect(
      transition(restoring, { lifecycle: "workspace_loading" }).lifecycle,
    ).toBe("workspace_loading");
  });

  it("rejects a direct ready-to-startup-error jump", () => {
    expect(() =>
      transition(modelAt("ready"), { lifecycle: "startup_error" }),
    ).toThrow(/ready -> startup_error/);
  });
});
